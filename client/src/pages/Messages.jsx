      ...
      <div className="max-w-6xl mx-auto p-6">
        ...
        <div className="flex flex-col gap-4">
          {connections.map((user) => (
            <div
              key={user._id}
              className="flex flex-col sm:flex-row items-start sm:items-center gap-4 sm:gap-5 p-6 bg-[#f9f7f6] border border-gray-400/25 shadow-lg rounded-2xl hover:shadow-xl transition transform hover:scale-[1.02] w-full"
            >
              {/* Avatar */}
              <img
                src={user.profile_picture}
                alt=""
                className="rounded-full w-16 h-16 shadow-md transition-transform transform hover:scale-105 flex-shrink-0"
              />

              {/* User Info */}
              <div className="flex-1 min-w-0">
                <p className="font-semibold text-[#2a2a2a] truncate">{user.full_name}</p>
